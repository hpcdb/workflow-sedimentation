...
void XDMFWriter::set_file_name(const char* filename)
{
...
void XDMFWriter::set_dir_path(const char* path)
{
    this->dir = path;
    std::cout << "[XDMF][SET_DIR_PATH] " << this->dir << endl;
    struct stat st = {0};
    if (stat(this->dir, &st) == -1) {
        mkdir(this->dir, 0700);
    }
}
...
//#define DEBUG
    ...
    char stepdir[80];
    sprintf(stepdir,"%s/step%d", this->dir, this->n_timestep);
    struct stat st = {0};
    ...
    sprintf(filename,"%s/%s_%d_%03d_%05d.h5",stepdir,this->basename,n_processors,processor_id, this->n_timestep);
    ...
    sprintf(filename,"%s/%s.xyz.%d.%03d.%05d.bin",stepdir,this->basename,n_processors,processor_id, this->n_timestep);
    BIN_WriteDouble(filename,&coords[0],coords.size());
    sprintf(filename,"%s/%s.con.%d.%03d.%05d.bin",stepdir,this->basename,n_processors,processor_id, this->n_timestep);
    ...
    for(int ns=0; ns < es.n_systems(); ++ns)
    {
        ...
        for(int nv=0; nv < es.get_system(ns).n_vars(); ++nv )
        {
            ...
            sprintf(filename,"%s/%s.%s.%d.%03d.%05d.bin",stepdir,this->basename,v.c_str(),
    ...
    string xdmf_filename = write_spatial_collection(es, time);
...
string XDMFWriter::write_spatial_collection(EquationSystems& es, double time)
{
    ...
    if(processor_id == 0)
    {
        ...
        sprintf(stepdir,"%s/step%d", this->dir, this->n_timestep);
        char stepfile[80];
        sprintf(stepfile,"step%d/%s",this->n_timestep ,this->basename);
        //sprintf(h5file,"%s",this->basename);
       
        sprintf(filename,"%s/%s_%d_%05d.xmf",this->dir ,this->basename, n_processors,this->n_timestep);
...
void XDMFWriter::write_temporal_collection()
{
    ...
    {
        char filename[255];   
        sprintf(filename,"%s/%s_%d.xmf", this->dir,this->basename, n_processors);
        FILE * fxml = fopen(filename, "w");
        fprintf(fxml,"<?xml version=\"1.0\" ?>\n");
        fprintf(fxml,"<!-- DOCTYPE Xdmf SYSTEM \"Xdmf.dtd\" [] --> \n");
        fprintf(fxml,"<Xdmf xmlns:xi=\"http://www.w3.org/2001/XInclude\" Version=\"2.0\"> \n");
