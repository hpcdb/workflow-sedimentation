  ...
  for ( ; el != end_el; ++el)
    {
      ...
      // The carateristic height of the element
        ...
        const Real h_caract    = 2.0*std::pow(volume*invPi, 0.5);
        const Real aux1        = 9.0*std::pow(4.0*k/(h_caract*h_caract),2.0)+ 4.0/(dt*dt);
        ...
        for (unsigned int qp=0; qp<qrule.n_points(); qp++)
        {
            ...
            Real tau         = compute_tau_M(g,G,U,k,dt,1.0);
            Real unorm       = U.norm();
            ...
            RealVectorValue velocity(u-tau*rint_x,v-tau*rint_y-Us);
            
            unorm            = velocity.norm();
            Real aux2        = std::pow(2.0*unorm/h_caract,2.0) + aux1;
            tau              = std::pow(aux2,-0.5);
          ...
          if (elem->neighbor(s) == NULL)
            {
              ...
              if(mesh.boundary_info->boundary_id(elem,s) == this->erosion_bc_id)
              {
                  ...
                  for (unsigned int qp=0; qp<qface.n_points(); qp++)
               {
                   ...
                   // RHS contribution
                   for (unsigned int i=0; i<phi_face.size(); i++)
                       Fe(i) += JxW_face[qp]*dt*Us*E*phi_face[i][qp];
