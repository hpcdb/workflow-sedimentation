...
namespace
{
  ...
  void BuildVTKGrid(EquationSystems &eq, std::map<int, int> &g2l)
  {
    ...
    if(elem_type == TRI3)
        VTKGrid->Allocate(static_cast<vtkIdType>(numberOfCells*4));
    else {
        if(elem_type == QUAD4 || elem_type == TET4)
    ...
    for ( ; el != end_el; ++el)
    {
        ...
        switch(elem->type()) {
            case TRI3:
                VTKGrid->InsertNextCell(VTK_TRIANGLE     , 3, tmp);
                break;
            case QUAD4:
                VTKGrid->InsertNextCell(VTK_QUAD        , 4, tmp);
                break;
            case TET4:
                VTKGrid->InsertNextCell(VTK_TETRA        , 4, tmp);
                break;
            default:
  ...
  void get_variable_solution(EquationSystems& es, int sys, int ivar, std::vector<double> &solution, std::map<int, int> &g2l)
{
  ...
  void BuildVTKDataStructures(EquationSystems &eq, std::map<int, int> &g2l)
  {
...
namespace FEAdaptor
{
  ...
  void CoProcess(EquationSystems &eq, double time,
                 unsigned int timeStep, bool lastTimeStep = false)
  {
