...
for i in `echo $lines`; do 
  host=`echo $i`
  echo "cd $cpath;monetdbd start $datapath;monetdbd get all $datapath;monetdb start dataflow_analyzer;monetdb status;$JAVA_HOME/bin/java -jar $cpath/../dfa/DI-1.0.jar -daemon start"
  cd $cpath;monetdbd start $datapath;monetdbd get all $datapath;monetdb start dataflow_analyzer;monetdb status;$JAVA_HOME/bin/java -jar $cpath/../dfa/DI-1.0.jar -daemon start
