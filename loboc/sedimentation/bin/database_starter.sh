...
for i in `echo $lines`; do 
  ...
  ssh $host "cd $cpath;monetdb-stop-all;killall monetdb;killall monetdbd;killall mserver5;monetdbd set port=54321 $datapath; monetdbd start $datapath;monetdbd get all $datapath;monetdb start dataflow_analyzer;monetdb status;java -jar /home/user4/sedimentation/dfa/DI-1.0.jar -daemon start" &
done